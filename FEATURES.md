...
| Feature | Status | Evidence / note |
|---|---|---|
| **Level select + a 2nd level (grocery store)** | **NOT BUILT — specced, Phase 0 done** | Kyle's current direction. Phase 0 measured the coupling: the world is 76 walls / 23 desks / 20 objects / 29 containers / 17 rooms / 19 NPCs at 1500x760, and `desks` alone has 114 references. Three findings make it cheap: `desks` already uses the populate-an-empty-array pattern, the arrays are `const` *bindings* (contents can be swapped without touching a single reference), and **a fresh run already reloads the page** carrying `promo:newgame`/`promo:char` — which is the level-swap mechanism, already built and trusted. Hard constraint: `scaleWorld()` scales in place and must run exactly once, so the order is read-flag → `loadLevel` → `scaleWorld` → `buildGrid`. **`SPEC-levels.md`**. |

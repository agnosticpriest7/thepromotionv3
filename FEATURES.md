...
| **Save-Rite floor plan (placeholder art)** | **SHIPPED** 2026-08-05 | Ten zones authored in the 1240x760-style authored space and scaled once: FRONT END, GROCERY, PRODUCE, BAKERY, DELI, and a walled back of house (RECEIVING, BREAK ROOM, STORE MANAGER, OWNER, STAFF WC) reached through one 90-unit opening. Entrance in the store's south wall with a bounded vestibule behind it. Fixtures are existing art only: `supply_shelf` shelf runs (61x88 authored, pitched 96 down / 150 across for ~89-unit aisles), `fridge` produce cases flush to the west wall, `filing_cabinet`/`lockers` in back, and office tables standing in for counters and checkouts. Sign reads **MERV'S**, canvas text. |
| ↳ **`fitTableSeats` was the last office leak — and the nastiest** | **FIXED 2026-08-05** | It was hardcoded to the five office table consts: it took their `crewRect`, made BLOCKERS of them and baked them into the nav grid, in *every* level. In Save-Rite that put a solid, undrawable meeting table across a grocery aisle. Invisible three ways — nothing renders it, `placement.js` lints props not blockers, and it waits on art to load so it appears *after* boot, not at it. Now driven off `LEVEL_TABLES` like `renderInner`. This is the layer *underneath* the guard added last branch. |
| ↳ `LEVEL_TABLES` / `levelBoxes` / `levelSpawn` | SHIPPED | The `renderInner` guard became "draw the current level's furniture" rather than "don't draw furniture in grocery". `levelBoxes` holds rects a level *creates* so `scaleWorld` scales them exactly once; the office's five tables are only *referenced*, never pushed, or they would double-scale. `levelSpawn` records where a level puts the player, because a test cannot safely guess it. |
| ↳ `placement.js` lints BOTH worlds | SHIPPED | `buildContext(opts)` threads the `createWorld({storage})` seed through; the CLI runs office then grocery and fails on either. **0 FAIL each** (office 72 props / 0 WARN, grocery 22 props / 6 WARN — the WARNs are island shelving, which is what an aisle is). |
| ↳ office TASKS appear in the store | **NOT BUILT (finding, now visible)** | `rollTasks` was on the clock list as theory; with a floor to stand on it is on screen — the TODAY tracker offers "Finish the TPS cover sheets at your desk" and "Restock the supply station" in a grocery store. Harmless, and the most visible piece of office fiction leaking through. |
| ↳ **office code that FIRES ON THE CLOCK in grocery** | **the list** | Measured by wrapping each function over a 5-day soak, not by reading. **Fires (call count over 5 days):** `checkMeeting` (150000 — every frame, measuring distance to a hardcoded office meeting table) · `tickLights` (150000) · `unstickPlayer` (150006) · `rollTasks` (17) · `maybeOfferFavor` (17) · `delegArrive` (17) · `cosignArrive` (17) · `clearSeats` (20) · `assembleAtElevator` (4) · `missMeeting` (4) · `nextDay` (4) · `autosave` (4) · `scheduleBoss` (4) · `scheduleAudit` (4) · `scheduleDrill` (4) · `triggerAudit` (4) · `rollContainers` (4) · `assignSeats` (4) · `rivalsClimb` (4) · `cosignRollDay` (4) · `delegRollDay` (4) · `resetLights` (4) · `processHires` (4) · `seedFetchNeeds` (4) · `buildGrid` (2). **Confirmed dormant:** `startIntro`, `resolveAudit`, `meltdown`, `startMeltEvent`, `pullAlarm`. Dormant-because-empty is not the same as safe — `triggerAudit` fires and finds no desk; give grocery desks and `resolveAudit` starts running office logic. |
| ↳ `tickLights` crashed every frame from 08:15 | **FIXED 2026-08-05** | `order` is a fixed index list `[0,4,1,5,…]` into `LIGHT_BANKS`, written for the office's ten banks; grocery has none, so `undefined.on` threw **every frame** — and because `advanceClock()` runs *after* `tickLights()` in the same block, the throw **froze the clock**. That is why no grocery day had ever rolled. Second bug on the next line: `reduce(...)/LIGHT_BANKS.length` is `0/0` = NaN with no banks. A level with no authored lighting is now treated as fully lit. |
| ↳ `startGame` relabelled the world it was in | **FIXED 2026-08-05** | The no-reload fallback adopted the level the menu had picked, so `startNewGame` in a grocery world set `currentLevel='office'` while the floor stayed grocery — the next save would claim a world it did not hold. The fallback no longer touches `currentLevel`. |
| ↳ `missMeeting` fires daily in grocery | **NOT BUILT (finding)** | It runs every day and is harmless only because the player is rank 0 (`missMeeting` returns early below rank 2). At rank ≥2 you would be disciplined for missing an office meeting that does not exist in a grocery store. Dormant by RANK, not by level. |
